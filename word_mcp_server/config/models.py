...
class ServerConfig(BaseModel):
    ...
    max_concurrent_docs: int = Field(
        default=10, description="Maximum concurrent documents"
    )
...
class WordConfig(BaseModel):
    ...
    auto_launch: bool = Field(
        default=True, description="Auto-launch Word if not running"
    )
    ...
    backup_directory: Optional[str] = Field(
        default=None, description="Backup directory path"
    )
...
class LoggingConfig(BaseModel):
    ...
    file: Optional[str] = Field(
        default="word_mcp_server.log", description="Log file path"
    )
    max_size_mb: int = Field(default=100, description="Maximum log file size in MB")
    backup_count: int = Field(default=5, description="Number of backup log files")
    console: bool = Field(
        default=False,
        description="Enable console logging (disabled for MCP compatibility)",
    )
    format: str = Field(
