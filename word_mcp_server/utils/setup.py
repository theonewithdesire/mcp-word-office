...
class SetupManager:
    ...
    def _check_python_packages(self) -> List[str]:
        ...
        required_packages = ["mcp", "pydantic", "pyyaml", "python-docx"]
        ...
        claude_config = {
            "mcpServers": {
                "word-office": {
                    ...
                    "env": {"WORD_MCP_CONFIG": config_path},
                }
            }
        }
...
"""
    ...
    def _provide_requirement_suggestions(self, issues: List[str]) -> None:
        ...
        for issue in issues:
            if "Python" in issue and "required" in issue:
                ...
                print(
                    "  • Consider using Windows Subsystem for Linux (WSL) with Windows"
                )
                ...
                print(
                    "  • Or install individually: pip install mcp pydantic pyyaml python-docx pywin32"
                )
    ...
    def _run_final_validation(self, config_path: str) -> Dict[str, any]:
        ...
        validation_results = {"success": True, "warnings": []}
        ...
        try:
            ...
            if self.is_windows:
                try:
                    ...
                    validation_results["warnings"].append(
                        f"Word COM interface test failed: {e}"
                    )
            ...
            try:
                ...
                validation_results["warnings"].append(
                    f"Configuration directory not writable: {e}"
                )
...
def main():
    ...
    parser.add_argument(
        "--uninstall", action="store_true", help="Uninstall Word MCP Server"
    )
    parser.add_argument(
        "--config-only", action="store_true", help="Create configuration only"
    )
