...
class SecurityLogger:
    ...
    def log_access_attempt(
        ...
        message = (
            f"File access {'granted' if success else 'denied'}: {operation} on {path}"
        )
    ...
    def log_permission_check(
        self, resource: str, permission: str, granted: bool, reason: str = None
    ):
...
def setup_logging(config: LoggingConfig) -> logging.Logger:
    ...
    if config.console:
        ...
        if (
            hasattr(console_handler.stream, "isatty")
...
def configure_graceful_degradation():
    ...
    fallback_handler.setFormatter(
        logging.Formatter("FALLBACK: %(asctime)s - %(levelname)s - %(message)s")
    )
