...
from .errors import WordMCPError, ErrorCode, ConnectionError, DocumentError, OperationError
...
class GracefulDegradation:
    ...
    def register_recovery_strategy(self, error_code: str, strategy: RecoveryStrategy, 
        ...
        self.logger.debug(f"Registered recovery strategy {strategy.value} for error: {error_code}")
        ...
        try:
            ...
            if operation in self.fallback_handlers:
                try:
                    ...
                    self.logger.error(f"Fallback also failed for {operation}: {fallback_error}")
...
class RecoveryOrchestrator:
    ...
    def register_recovery_action(self, trigger: str, action_func: Callable, 
                               priority: int = 0, max_concurrent: int = 1):
            ...
            try:
                ...
                for check_name, status in health_status["checks"].items():
                    if not status["healthy"]:
                        await self.trigger_recovery(f"unhealthy_{check_name}", {
