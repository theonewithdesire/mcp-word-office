    ...
    def __init__(
        ...
        if not self.suggestions:
            try:
                ...
                self.suggestions = ErrorSuggestions.get_suggestions(
...
class ConnectionError(WordMCPError):
    ...
    def __init__(
        self, message: str = "Failed to connect to Word application", **kwargs
    ):
    ...
    def _attempt_recovery(self, error: WordMCPError) -> Optional[Dict[str, Any]]:
        ...
        if recovery_strategy:
            try:
                ...
                self.logger.warning(
                    f"Recovery strategy failed for {error.error_code}: {e}"
                )
        return None
