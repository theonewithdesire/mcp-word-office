...
Examples:
  ...
  word-mcp-server --create-config         # Create default config file
    ...
    parser.add_argument(
        "--check-requirements", action="store_true", help="Check system requirements"
    )
...
class ServerManager:
    ...
    async def start_server(
        ...
        try:
            ...
            self.logger.info(
                f"Configuration loaded from: {config_manager.get_config_path() or 'defaults'}"
            )
            self.logger.info(
                f"Server config - Host: {config.server.host}, Port: {config.server.port}"
            )
            self.logger.info(
                f"Max concurrent docs: {config.server.max_concurrent_docs}"
            )
            self.logger.info(
                f"Word auto-launch: {config.word.auto_launch}, visible: {config.word.visible}"
            )
            ...
            self.logger.info(
                f"Allowed paths: {len(config.security.allowed_paths)} configured"
            )
