...
from ..utils.errors import (
    ConnectionError, DocumentError, OperationError, ErrorCode, 
    WordMCPError, TimeoutError
)
from ..utils.logging import get_logger, get_performance_logger, get_security_logger
from ..utils.recovery import CircuitBreaker, RetryManager, RecoveryConfig
...
class WordController:
    ...
    def __init__(self, config: WordConfig):
        ...
        self.retry_manager = RetryManager(RecoveryConfig(
            max_retries=3,
            retry_delay=2.0,
    ...
    async def cleanup_all_documents(self):
        ...
        """
        self.logger.info(f"Starting document cleanup for {len(self._documents)} documents")
        ...
        for doc_id in doc_ids:
            try:
                ...
                if doc_ref:
                    self.logger.debug(f"Cleaning up document: {doc_ref.title} ({doc_id})")
                    
                    # Save document if it has unsaved changes
                    if doc_ref.word_doc_ref:
                        try:
                            # Check if document has unsaved changes
                            if hasattr(doc_ref.word_doc_ref, 'Saved') and not doc_ref.word_doc_ref.Saved:
                                if doc_ref.file_path:
                                    doc_ref.word_doc_ref.Save()
                                    self.logger.debug(f"Saved document: {doc_ref.title}")
                                else:
                                    # Document has no path, save with default name
                                    default_path = f"Document_{doc_id}.docx"
                                    doc_ref.word_doc_ref.SaveAs2(default_path)
                                    self.logger.debug(f"Saved new document as: {default_path}")
                        except Exception as save_error:
                            error_msg = f"Failed to save document {doc_ref.title}: {save_error}"
    ...
    def insert_text(self, doc_id: str, text: str, position: Optional[int] = None):
        ...
        try:
            ...
            self.logger.info(f"Inserted text into document {doc_id} at position {position}")
    ...
    def format_text(self, doc_id: str, start: int, end: int, **formatting):
        ...
        try:
            ...
            if formatting.get('highlight_color'):
                # Set highlight color
                highlight_value = self._parse_highlight_color(formatting['highlight_color'])
                if highlight_value is not None:
                    text_range.HighlightColorIndex = highlight_value
            
            doc_ref.last_modified = datetime.now()
            
            self.logger.info(f"Applied formatting to document {doc_id} range {start}-{end}")
    ...
    def create_table(self, doc_id: str, rows: int, cols: int, position: Optional[int] = None) -> Dict[str, Any]:
    ...
    def format_table_cell(self, doc_id: str, table_index: int, row: int, col: int, 
                         text: Optional[str] = None, **formatting) -> None:
        """Format a table cell.
        ...
        try:
            ...
            if table_index < 1 or table_index > doc.Tables.Count:
                raise OperationError(f"Table index {table_index} out of range (1-{doc.Tables.Count})")
            ...
            if col < 1 or col > table.Columns.Count:
                raise OperationError(f"Column {col} out of range (1-{table.Columns.Count})")
    ...
    def create_list(self, doc_id: str, items: List[str], list_type: str = "bulleted", 
        ...
        try:
            ...
            self.logger.info(f"Created {list_type} list with {len(items)} items in document {doc_id}")
    ...
    def find_replace(self, doc_id: str, find_text: str, replace_text: str, 
                    match_case: bool = False, match_whole_word: bool = False, 
                    use_regex: bool = False, replace_all: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Find/replace operation in document {doc_id}: {replacements_made} replacements made")
    ...
    def insert_header_footer(self, doc_id: str, header_text: Optional[str] = None, 
                           footer_text: Optional[str] = None, section_index: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            if header_text is not None:
                try:
                    ...
                    self.logger.info(f"Inserted header in document {doc_id}, section {section_index}")
                except Exception as e:
                    results["header"] = {
                        "success": False,
                        "error": str(e)
            ...
            if footer_text is not None:
                try:
                    ...
                    self.logger.info(f"Inserted footer in document {doc_id}, section {section_index}")
                except Exception as e:
                    results["footer"] = {
                        "success": False,
                        "error": str(e)
    ...
    def insert_page_break(self, doc_id: str, position: Optional[int] = None, 
                         break_type: str = "page") -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Inserted {break_type} break in document {doc_id} at position {position}")
    ...
    def set_page_formatting(self, doc_id: str, section_index: int = 1, 
        ...
        try:
            ...
            if margins:
                try:
                    ...
                    self.logger.info(f"Set margins for document {doc_id}, section {section_index}")
            ...
            if orientation:
                try:
                    if orientation.lower() == "portrait":
                        ...
                        results["orientation_error"] = f"Invalid orientation: {orientation}"
                    
                    self.logger.info(f"Set orientation to {orientation} for document {doc_id}, section {section_index}")
            ...
            if paper_size:
                try:
                    ...
                    if paper_constant:
                        ...
                        self.logger.info(f"Set paper size to {paper_size} for document {doc_id}, section {section_index}")
                    else:
                        results["paper_size_error"] = f"Invalid paper size: {paper_size}"
