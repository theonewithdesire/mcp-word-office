...
class WordMCPServer:
        ...
        if self.word_controller is None:
            try:
                ...
                if "COM interface not available" in str(e):
                    logger.warning(
                        "COM interface not available, using mock controller for testing"
                    )
    ...
    async def _handle_save_document(self, args=None, **kwargs):
        ...
        path = args.get("path")
        if not doc_id:
            return {"success": False, "error": "doc_id parameter is required"}
        try:
            controller = self._ensure_word_controller()
    ...
    async def _handle_format_text(self, args=None, **kwargs):
        ...
        try:
            ...
            formatting_args = {
                k: v for k, v in args.items() if k not in ["doc_id", "start", "end"]
            }
    ...
    async def _handle_format_table_cell(self, args=None, **kwargs):
        ...
        try:
            ...
            result = controller.format_table_cell(
                doc_id, table_index, row, col, text, **formatting_args
            )
            return {
                "success": True,
                "doc_id": doc_id,
    ...
    async def _handle_find_replace(self, args=None, **kwargs):
        ...
        try:
            ...
            result = controller.find_replace(
                doc_id, find_text, replace_text, **formatting_args
            )
            return {
                "success": True,
                "doc_id": doc_id,
    ...
    async def _handle_insert_header_footer(self, args=None, **kwargs):
        ...
        try:
            controller = self._ensure_word_controller()
            result = controller.insert_header_footer(
    ...
    async def _handle_set_page_formatting(self, args=None, **kwargs):
        ...
        try:
            ...
            # Filter out non-formatting arguments
            formatting_args = {
                k: v for k, v in args.items() if k not in ["doc_id", "section_index"]
            }
            result = controller.set_page_formatting(
                doc_id, section_index, **formatting_args
            )
            return {
                "success": True,
                "doc_id": doc_id,
